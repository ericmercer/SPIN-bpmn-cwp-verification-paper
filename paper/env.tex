...
The verification model assumes the \texttt{orders} from an examining physician correlate strongly with \texttt{sevLvl}. A \texttt{sevLvl} below two does not require hospitalization according to the DRG scale while anything above two is the opposite.
%
{\small
...
Of note is that \texttt{sevLvl} may never be zero! It is possible for the model checker to resolve the non-determinism in such a way that \texttt{sevLvl} is always one. The result is that the patient may never be discharged and may never expire at the same time. Such behavior exists in the model but is not consistent with the real world. These are part of the sound over-approximation, and when necessary, are constrained out by changing the model or constraining what behaviors are considered in the model with a fairness property as discussed previously.
