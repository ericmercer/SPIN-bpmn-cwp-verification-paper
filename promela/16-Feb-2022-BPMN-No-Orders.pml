...
inline clinicianTask01UpdateState() {
  assert(!meetsDischargeCriteria(sevNeed) && !isExpired(sevNeed))
  
  if
  :: true -> setWithinHomeCare(sevNeed)
  :: (isInit(sevNeed) || isOutsideHomeCare(trndSevNeed)) -> 
    setOutsideHomeCare(sevNeed)
  :: (!isInit(sevNeed) && isWithinHomeCare(trndSevNeed)) -> setDischargeCriteria(sevNeed)
  fi 
  trndSevNeed = sevNeed
  updateState()
}
...
inline clinicianTask03UpdateState() {
  ...
  :: true -> setDischargeCriteria(sevNeed)
  fi 
  trndSevNeed = sevNeed
  updateState()
}
...
active proctype clinician() {
  ...
  :: // Clinician Xor4
    atomic {
      ...
      :: meetsDischargeCriteria(sevNeed) ->
